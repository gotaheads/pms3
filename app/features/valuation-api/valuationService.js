  ...
  .service('valuationService', ['$http', '$log', '$q', '$rootScope',
    ...
    function valuationService($http, $log, $q, $rootScope,
      ...
      valuationService.pdf = function(year, number) {
        ...
        var url = createValuationUrl('/generators?year=' + year + '&number=' + number);
      ...
      valuationService.sendAll = function(year, sending, landlordsToSend, startBy, sendAllStatus) {
        return sendAll.start(year, sending, landlordsToSend, startBy, sendAllStatus).then(function (sendAllStatusCompleted) {
          $log.info('valuationService.sendAll  sendAllStatusCompleted: ', sendAllStatusCompleted);
          return sendAllStatusCompleted;
        });
      }
